...
function evaluate_policies(βs, τmax)
    ...
    @showprogress for β in βs
        ...
        @time results[β]["receding_h2"] =
            evaluate(mdp, RecedingHorizonPolicy(mdp, 2)).(states(mdp))
        @time results[β]["receding_h3"] =
            evaluate(mdp, RecedingHorizonPolicy(mdp, 3)).(states(mdp))
